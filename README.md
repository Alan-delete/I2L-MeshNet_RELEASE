# Application and experiments based on "I2L-MeshNet: Image-to-Lixel Prediction Network for Accurate 3D Human Pose and Mesh Estimation from a Single RGB Image "
...
<p align="middle">
<img src="" width="260" height="160"><img src="" width="260" height="160"><img src="" width="260" height="160">
</p>

## Introduction  
This repo is the simple application of I2L-MeshNet(Reference below). We build a fitness advice web application using the joints coordinates generated by simplied I2L-MeshNet. Also, we tried Sem_GCN to replace coordinate regression convolutional layers in original Network and compare the result in a intuative way through our web application.
...
|-- weights
```  
* `app` contains web application codes.
* `data` contains data loading codes and soft links to images and annotations directories.  
* `demo` contains orignal demo codes but won't be used in this project.
* `common` contains kernel codes for I2L-MeshNet.  
* `main` contains high-level codes for training or testing the network.  
* `weights` contains checkpoints downloading script and provide folder for users' training checkpoints.  
...
* There are two stages. 1) `lixel` and 2) `param`. In the `lixel` stage, I2L-MeshNet predicts lixel-based 1D heatmaps for each human joint. In `sem_gcn` stage, I2L-MeshNet use sem_gcn to lift 2D coordinated to 3d coordinates.
...
#### 2. sem_gcn stage
Once you pre-trained I2L-MeshNet in `lixel` stage, you can resume training in `sem_gcn` stage. In the `main` folder, run  
```bash  
python train.py --gpu 0-3 --stage sem_gcn --continue
```  
to train I2L-MeshNet in the `sem_gcn` stage on the GPU 0,1,2,3. `--gpu 0,1,2,3` can be used instead of `--gpu 0-3`.  
  
### Test  
Place trained model at the `output/model_dump/`.  Choose the stage you want to test among `lixel` and `sem_gcn`.
...
### Web application
Go to the directory `app/server/`.  Choose the stage you want to run among `lixel` and `sem_gcn`.
  
In the `server` folder, run  
```bash  
python3 app.py --gpu 0-3 --stage $STAGE
```  
to test I2L-MeshNet in `$STAGE` stage (should be one of `lixel` and `param`) on the GPU 0,1,2,3. `--gpu 0,1,2,3` can be used instead of `--gpu 0-3`.  
